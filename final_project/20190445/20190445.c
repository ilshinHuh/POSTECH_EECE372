...
void Conv_2d(float *feature_in, float *feature_out, int in_C, int in_H, int in_W, int out_C, int out_H, int out_W, int K, int S, float *weight, float *bias) {
    ...
    int out_height, out_width, out_channel, in_channel, kernal_height, kernal_width;
    ...
    for (out_channel = 0; out_channel < out_C; out_channel++) {
        for (out_height = 0; out_height < out_H; out_height++) {
            for (out_width = 0; out_width < out_W; out_width++) {
                float kernal_sum = 0.0f;
                for (in_channel = 0; in_channel < in_C; in_channel++) {
                    for (kernal_height = 0; kernal_height < K; kernal_height++) {
                        for (kernal_width = 0; kernal_width < K; kernal_width++) {
                            kernal_sum += feature_in[in_channel * in_H * in_W + (out_height * S + kernal_height) * in_W + out_width * S + kernal_width]\
                            * weight[out_channel * in_C * K * K + in_channel * K * K + kernal_height * K + kernal_width];
                        }
                    }
                }
                kernal_sum += bias[out_channel];
                feature_out[out_channel * out_H * out_W + out_height * out_W + out_width] = kernal_sum;
            }
        }
    }
...
void Linear(float *feature_in, float *feature_out, float *weight, float *bias) {
    ...
    for (int i = 0; i < FC_OUT; i++) {
        ...
        for (int j = 0; j < FC_IN; j++) {
            sum += feature_in[j] * weight[i * FC_IN + j];
        }
        sum += bias[i];
        feature_out[i] = sum;
    }
