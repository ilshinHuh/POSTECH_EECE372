...
int main(int argc, char *argv[]) {
    clock_t start1, end1, start2, end2, clock0, clock1, clock2, clock3, clock4, clock5, clock6, clock7;
    ...
    printf("Padding 1 Execution time: %9.3lf[us]\n", (double)(clock1 - clock0) / CLOCKS_PER_US);
    printf("Conv 1 Execution time: %9.3lf[us]\n", (double)(clock2 - clock1) / CLOCKS_PER_US);
    printf("Relu 1 Execution time: %9.3lf[us]\n", (double)(clock3 - clock2) / CLOCKS_PER_US);
    printf("Padding 2 Execution time: %9.3lf[us]\n", (double)(clock4 - clock3) / CLOCKS_PER_US);
    printf("Conv 2 Execution time: %9.3lf[us]\n", (double)(clock5 - clock4) / CLOCKS_PER_US);
    printf("Relu 2 Execution time: %9.3lf[us]\n", (double)(clock6 - clock5) / CLOCKS_PER_US);
    printf("Linear Execution time: %9.3lf[us]\n", (double)(clock7 - clock6) / CLOCKS_PER_US);
    printf("pred, CAM Execution time: %9.3lf[us]\n", (double)(end2) / CLOCKS_PER_US);
    /************************************************************************/
...
void Conv_2d(float *feature_in, float *feature_out, int in_C, int in_H, int in_W, int out_C, int out_H, int out_W, int K, int S, float *weight, float *bias) {
    ...
    "out_channel_loop: \n\t"
        "cmp r0, r3\n\t"
        "bge out_channel_loop_done\n\t"
        
        "mov r1, #0\n\t" // out_height
    "out_height_loop: \n\t"
        "cmp r1, r4\n\t"
        "bge out_height_loop_done\n\t"
        
        "mov r2, #0\n\t" // out_width
    "out_width_loop: \n\t"
        "cmp r2, r5\n\t"
        "bge out_width_loop_done\n\t"
    ...
    "out_height_loop_done: \n\t"
        "add r0, r0, #1\n\t"
        "b out_channel_loop\n\t"
    
    "out_channel_loop_done: \n\t"
        
    "end: \n\t"
        :
        : [weight] "r" (weight), [feature_in] "r" (feature_in), [bias] "r" (bias), [feature_out] "r" (feature_out),
        [in_W] "m" (in_W), [K] "m" (K),  [in_H] "m" (in_H), [in_C] "m" (in_C), [S] "m" (S), 
        [out_C] "m" (out_C), [out_H] "m" (out_H), [out_W] "m" (out_W)
        : "r0", "r1", "r2", "r3", "r4", "r5", "r6", "r8", "s0", "s1", "s2", "pc", "lr", "memory"
    );
